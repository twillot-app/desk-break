...
- **Exercises + media (v0.4)**: `MEDIA_BASE_URL` (raw base for the demo GIFs); `FOCUS_PARTS` (comma list of groups); `FOCUS_WEIGHT` (priority multiplier, default 3); `FOCUS_COOLDOWN` (consecutive focus shows before forcing a non-focus, default 2); `EXERCISE_RECENT_K` (avoid repeating the last K, default 6). An exercise is always offered; the demo card loads (and makes a network request) only when the user clicks **See demo**.
